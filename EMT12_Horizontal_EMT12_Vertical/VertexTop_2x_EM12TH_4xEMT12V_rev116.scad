...
//6:31:38 print time at 50% infill, 2.82oz weight
//$vpt=[-4.8,-22,25.6];
//$vpr =[55,0,71.2];
//$vpd =435;
height = 43;
...
EMT_OD = 18.0;                     //Originally used 18.4, 18.2 was also a little loose
...
//Cutouts for idler & mounting hardware
...
//Chamfer the top & bottom of side cuts for no overhangs at top of cuts
        ...
        for (x = [-1,1])
        {
            translate([x * 1.5,-18,-1])
            cylinder(d=5,h=200,$fn=4);
        }
    }
}
...
module EdgePipes()
{
    Sh_Len = 5;         //5mm, Shorten Bottom cut length by this much
                        //Change to 10 for test print
